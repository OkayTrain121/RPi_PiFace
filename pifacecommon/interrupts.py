...
class InterruptEvent(object):
    ...
    def __init__(
        ...
        logger("Constructing InterruptEvent interrupt_flag : " + str(interrupt_flag) + " interrupt_cap : " + str(interrupt_capture))
...
class PinFunctionMap(FunctionMap):
    ...
    def __init__(self, pin_num, direction, callback, settle_time):
        logger("Constructing PinFunctionMap")
...
class EventQueue(object):
    ...
    def __init__(self, pin_function_maps):
        logger("Constructing EventQueue")
    ...
    def add_event(self, event):
        ...
        bouncing.
        """
        logger("Trying to add event : " + str(event))
        ...
        for pin_function_map in self.pin_function_maps:
            ...
            #         pin_function_map.direction == IODIR_BOTH):
                ...
                logger("EventQueue: Found event in map.")
                break
        # Is this indentation for the 'else' block correct?
        else:
            # Couldn't find event in map, don't bother adding it to the queue
            logger("EventQueue: Couldn't find event in map:")
            # for pin_function_map in self.pin_function_maps:
...
class PortEventListener(object):
    ...
    def __init__(self, port, chip, return_after_kbdint=True, daemon=False):
        logger("Constructing PortEventListener on Port : " + str(port))
        self.port = port
        self.chip = chip
    ...
    def register(self, pin_num, direction, callback,
        ...
        """
        logger("Registering Pin : " + str(pin_num) + " Dir : " + str(direction) + " Cb : " + str(callback))
...
class GPIOInterruptDevice(object):
    ...
    def gpio_interrupts_enable(self):
        ...
        try:
            ...
            logger("Enabled GPIO Interrupts")
        except Timeout as e:
    ...
    def gpio_interrupts_disable(self):
        ...
        logger("Disabled GPIO Interrupts")
...
def _event_matches_pin_function_map(event, pin_function_map):
    ...
    logger("Checking if event matches pin func map")
    ...
    logger("pin_match : " + str(pin_match) + " and direction_match : " + str(direction_match))
...
def watch_port_events(port, chip, pin_function_maps, event_queue,
    ...
    """
    logger("Watching port events on port :" + str(port) + ", chip : " + str(chip))
    logger("Configured ports :" + str(pifacecommon.mcp23s17.GPIOA) + " and " + str(pifacecommon.mcp23s17.GPIOB))
    # set up epoll
    ...
    while True:
        ...
        logger("Debug_count : " + str(debug_count))
        # wait here until input
        try:
            logger("1")
            events = epoll.poll(2)   # Poll with some timeout of 1sec 
            #events = epoll.poll()
            logger("Polled")
        except KeyboardInterrupt as e:
            if return_after_kbdint:
                logger("Returning from 1st try-expect")
                return
            else:
                logger("Raising in 1st try-expect")
                raise e
        except IOError as e:
            logger("2")
        ...
        if interrupt_flag == 0:
            ...
            logger("Watcher trying to add event")
            if port == pifacecommon.mcp23s17.GPIOA:
                logger("7")
            ...
            # Should we not break after adding event here?
    epoll.close()
...
#        logger("TEST!!!!")
        if event is not None:
            logger("HANDLE: It's an event")
        # print("HANDLE: It's an event!")
        else :
            logger("HANDLE: No event")
            time.sleep(2)            # Random wait
            continue
        if event == terminate_signal:
            logger("Recvd terminate signal")
            return
        ...
        for function in functions:
            logger("About to call the cb")
            function(event)
            logger("Returned after handling cb")
